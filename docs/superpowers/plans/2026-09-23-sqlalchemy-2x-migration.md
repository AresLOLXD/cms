...
- **Process note (found in Task 2):** `docker compose ... run --rm testcms ...` does NOT rebuild the image on its own — the source tree is `COPY`'d in at build time, not volume-mounted, so a `run` after a code edit silently reuses the stale image and any test result from it is meaningless. Every task from here on must `docker compose -p cms-beta -f docker/docker-compose.test.yml build testcms` (no `--no-cache` needed for an ordinary code-only edit — only Task 1's dependency-pin bump needed `--no-cache`) before each `run` that's meant to verify new code changes, not just before the task's very first baseline check.
