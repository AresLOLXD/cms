...
- Namespaces are fully isolated: stores, `ScoringStore`, SSE `DataWatcher`,
  logo, faces, flags and teams (each seeded at startup exactly as the root
  is today).
...
- `docker/docker-compose.prod.yml`: add a named volume for the ranking
  container's `lib_dir` (`/home/cmsuser/cms/lib/ranking`) so every
  namespace's data survives ranking-container restarts; without it a
  restarted ranking stays empty until **Regenerate** is used.
- In `ALL` mode `cmsTelegramBot` is not generated (a warning is printed);
  `docker/test_generate_config.py` covers it.
